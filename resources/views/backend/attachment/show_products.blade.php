        ...
        <table class="table table-bordered table-hover table-striped align-middle">
            ...
            <tbody>
                ...
                <tr  id="item-{{ $item->id }}">
                   ;
                    <td>{{ $item->id }}</td>
                    <td class="text-center mt-5">{{ $item->product_name }}</td>
                    <
                    <td>
                        @if($item->product_image)
                            <img class="rounded-circle" src="{{ asset('storage/' . $item->product_image) }}" alt="Banner Image" width="80" height="80" class="img-thumbnail">
                        @else
                            <span class="text-muted">No Image</span>
                        @endif
                    </td>
                    <td>
                        <a href="{{route('edit.products',['id' => $item->id])}}" class="btn btn-success">edit</a>
...
<script>
    $(document).on('click', '.btn-delete', function(e) {
        ...
        var url = "{{ route('delete.products', '') }}/" + id;  // Construct the URL with the route
